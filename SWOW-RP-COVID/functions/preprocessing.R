...
getMulti <- function(response,number) {
  if(!response %in% c(missing.Token,unknown.Token)){
    
    result = trimws(unlist(stri_split(response,regex = "[,]+")))
    if(length(result) >= number){
...
parseConcatenatedResponses = function(X,missing.Token,unknown.Token){
  ...
  # Are responses separated by a comma (updated 8/09/2022, SDD)
  X = X %>% mutate(nSplits = stri_count(R1, regex = "[,]+[^,]") + 1,
  ...
  X = X %>%  group_by(participantID) %>%
    mutate(propThreeResponses = sum(nWords >2,na.rm=T)/n(),
           serialResponder = propThreeResponses > criteria.nWords)
  
  X = X %>% rowwise() %>% mutate(R2 = ifelse(nSplits > 1, getMulti(R1,2),R2),
                                 R3 = ifelse(nSplits > 1, getMulti(R1,3),R3),
                                 R1 = ifelse(nSplits > 1, getMulti(R1,1),R1))
  
  X = X %>% ungroup %>% select(-propThreeResponses)
  nMultiResponsesChanged = sum(X$nWords > 1,na.rm = T)
...
normalizeCues = function(X,file.cueCorrections){
  ...
  X = left_join(X,X.cuecor)
  nCues = X %>% filter(cue!=cueCorrection) %>% group_by(cue) %>% tally() %>% nrow()
...
spellCheckResponse = function(X,file.lexicon,
