...
file.output   = paste0('../data/SWOW/output/responseStats.targetCues.csv')
...
Freq$R123      = X %>% group_by(response, cov.time, cue.type, cue) %>%
...
Freq$R1      = X  %>% filter(RPOS == 'R1') %>% 
  group_by(response, cov.time, cue.type, cue) %>%
...
# Combine R1 and R123
Freq.SWOW = right_join(Freq$R1,Freq$R123)

# Write the response statistics for both R1 and R123
write.csv(Freq.SWOW,file.output,row.names = FALSE, fileEncoding = 'UTF-8')
